...
> **明确边界**：`P_fill` 等“能否买到/成交性”信息 **只做标注**，不作为过滤与排序条件（0 权重影响排序）。
...
> 注意：`pred_source_latest.csv` **不需要**包含价格。价格由真值层提供。
...
- `p_fill` / `fill_hint`（可成交性：只标注，不过滤、不排序，允许为空）
- `dec_*`（决策层合并字段：只标注）
...
> 明确：`p_fill` 不参与排序；`dec_*` 不参与排序；任何风险提示字段不参与硬过滤。
...
## 10. V2 验收标准（锁死）
必须满足：
1) `premium_top30/full` 中 `close_T2_p50` 不再全空/全 0（在 close_T 可得时）
2) `target_date` 能在真值可得时正确推进到 T+2
3) 真值到达后 `premium_verify` 自动填充 `close_T2_actual` 与误差/覆盖字段
4) 校准指标能持续更新（准闭环成立）
5) `p_fill` 仅标注，不影响排序与过滤
