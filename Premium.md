明白。你现在这版 Premium.md 内容已经对了，我给你一个“**不改语义、只增强可读性**”的排版方案：用 **Markdown 分割线 `---`** 做大段分块，并给每块加统一风格标题。

下面是**可直接整段覆盖**到 Premium.md 的版本（已按你现有内容重排，增加分割线；不改变任何规则与条款）。
...
* outputs/premium/premium_top30_{T}.csv（Top30 预测表）
* outputs/premium/premium_full_{T}.csv（Full 预测表）
* outputs/premium/premium_verify_{T}.csv（验证表，顺序与 Top30 完全一致）
* docs/reports/premium_{T}.md（报告）
* docs/reports/premium_latest.md（每次覆盖）
* outputs/premium/_last_run.txt（运行追溯，每次覆盖）
...
* S0：N < 30：仅用全局历史分布（强收缩基线），保证稳定不断更
* S1：30 ≤ N < 60：单因子分桶（优先 p_premium 分位桶），桶内统计 + 收缩
* S2：60 ≤ N < 90：双因子分桶（p_premium × 强度/热度 任一稳定字段），桶内统计 + 收缩
* S3：90 ≤ N < 150：稳健线性/岭回归预测 r_p50，分位用残差分布估计
* S4：N ≥ 150：分位数回归模型（如 LGBM Quantile）直接输出 r_p05/p50/p95 等，并持续校准
...
* 口径统一：p_fill 定义固定、可复现、跨天一致（非临时手工/非随意变更）
* 真值可回流：能稳定获得执行真值（是否买到/成交价/滑点之一或组合），覆盖率足够，能形成有效样本 N（否则学习会变慢且偏）
* 目标明确升级：若 p_fill 进入中心预测或排序，必须将 Premium 的主目标从“价格分布预测”明确升级为“可执行收益/成交后收益分布预测”，并接受标签与校准口径变化
